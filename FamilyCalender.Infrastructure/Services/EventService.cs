...
namespace FamilyCalender.Infrastructure.Services
{
    ...
    public class EventService(ApplicationDbContext context, EncryptionService encryptionService) : IEventService
    {
        ...
        public async Task DeleteEventAsync(int eventId)
        {
            ...
            var calendar = eventToDelete.Calendar;
            if (calendar != null)
            {
        ...
        public async Task DeleteEventMemberDateAsync(int eventId, int memberId, DateTime date)
        {
            ...
            var calendar = eventDateToRemove.Event?.Calendar;
            if (calendar != null)
            {
                calendar.LastEditedUtc = DateTime.UtcNow;
        ...
        public async Task DeleteAllEventMemberDatesAsync(int eventId, int memberId)
        {
            ...
            var calendar = eventMemberDates.First().Event?.Calendar;
            if (calendar != null)
            {
                calendar.LastEditedUtc = DateTime.UtcNow;
                _context.Entry(calendar).Property(c => c.LastEditedUtc).IsModified = true;
            ...
            _context.EventMemberDates.RemoveRange(eventMemberDates);
            await _context.SaveChangesAsync();
        }
